(function () {
  ...
  // ---------------- Section versioning ----------------
  // A section may carry data-section-version. When an author edits a section
  // during a plan update they bump that value; the change below clears any
  // stored "read" flag for that section so the reader re-reads it. Sections
  // without a version are never auto-reset.
  function reconcileVersions() {
    var state = loadState();
    if (!state.__versions) state.__versions = {};
    var pageVersions = state.__versions[pageId] || {};
    var changed = false;

    checkboxes.forEach(function (cb) {
      var version = cb.dataset.sectionVersion;
      if (version == null) return;
      var sectionId = cb.dataset.sectionId;
      if (pageVersions[sectionId] !== version) {
        // Version differs (or is newly tracked) — reset read state for it.
        if (pageVersions[sectionId] !== undefined &&
            state[pageId] && state[pageId][sectionId]) {
          delete state[pageId][sectionId];
          if (Object.keys(state[pageId]).length === 0) delete state[pageId];
        }
        pageVersions[sectionId] = version;
        changed = true;
      }
    });

    if (changed) {
      state.__versions[pageId] = pageVersions;
      saveState(state);
    }
  }
